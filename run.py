...
@app.command(help="Run Alembic Migration")
def alembic_migrate(message: str = typer.Argument(..., help="Commit message")):
	try:
		activate_venv()
		...
		typer.echo(f"Error during generate alembic migration: {e}", err=True)

@app.command(help="Run Alembic Current Status")
def alembic_status():
	ctx_obj.config["wait"] = True
	try:
		activate_venv()
		subprocess.check_call(["alembic", "current"])
	except Exception as e:
		typer.echo(f"Error during get alembic status: {e}", err=True)

@app.command(help="Run Alembic Rollback")
def alembic_rollback(step: int = typer.Option(-1, help="Step to rollback")):
	if step > 0:
		typer.echo(f"❌ Step to rollback must be negative.")
		raise typer.Exit(1)
	else:
		try:
			activate_venv()
			subprocess.check_call(["alembic", "downgrade", str(step)])
			typer.echo(f"Rollback with step: {step}")
		except Exception as e:
			typer.echo(f"Error during rollback alembic: {e}", err=True)
